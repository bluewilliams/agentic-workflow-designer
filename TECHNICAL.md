...
- **Model**: Opus 4.8 (default), Opus 4.7, Opus 4.6, Sonnet 4.6, Sonnet 4.5, Opus 4.5, Haiku 4.5
...
**Generation:** `genDurableRecordProtocol()` emits the markdown protocol. The document's sections are deliberately on par with spec-driven tools (spec with testable scenarios, an approach-and-decisions section, a surface-area file-to-role map, a **work-breakdown task checklist** - the concrete coding/research/test units, explicitly not the workflow's agent/step names - a **Verify** section capturing the real build/test commands and results, and an outcome summary on completion) plus three things those tools tend to lack: a current-state/next-action section (which is what makes it a resumable handoff), a **Built-with provenance** line (workflow name + agent roles + grounding, kept in the commit as durable "how this was built" metadata for re-drive, evals, and audit), and the Verify results as an eval substrate. It also carries single-repo vs cross-repo destination guidance and an action-trigger ownership model (any agent amends the sections its step touched - decision to approach, files to surface area, build/tests to Verify, new work to the checklist, synthesizer to outcome + scaffolding strip). Provenance is durable and committed; transient minute-to-minute step progress lives only in the current-state scaffolding and is stripped on commit. `genDurableRecordComment(prefix)` emits a comment-prefixed pointer for the Python SDK exporter. Both are injected at the workflow-level memory site of all five exporters, nested inside the `memoryEnabled` block (so the superset holds even if state is inconsistent). It is a workflow-level artifact, not per-agent, so it is not injected into per-agent preambles/postambles.
