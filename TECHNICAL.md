...
- **Model**: Fable, Opus (default), Opus 4.8, Sonnet, Sonnet 4.6, Haiku (every family except Haiku also has a [1M] variant). Unnumbered labels mark the newest model of each family; numbered labels mark deliberately pinned legacy models - the label convention mirrors the alias-vs-full-id emission split below, and a naming-rule test enforces it. `taskModelMap` holds only the newest model in each family, so those emit the short alias (`fable`/`opus`/`sonnet`/`haiku`) and every superseded model falls through to its full API id. That fall-through is the point: leaving a superseded model in the map would make it emit a shorthand that now resolves to its successor, silently running the step on a different model. `modelContextNote` carries the 1M intent as prose and follows the same base, so a pinned model's note reads `/model claude-opus-4-8[1m]`
- **Effort**: per-node `config.effort` (`''` = inherit) baked at creation from `state.effort` (the sidebar Default Effort), mirroring how `model` is baked from `state.defaultModel`. Persistence is free: node effort rides `serializeWorkflow`/`deserializeWorkflow` with the rest of node config, the default rides `savePrefs`/`restorePrefs`, and `deserializeWorkflow` drops an unknown level the same way it drops an unknown model. `effortStepNote(node)` is a peer of `modelContextNote` emitted at the same five sites beside each step's Model line; `effortHint()` is the workflow-level beat that explains the session-flag mechanism once, gated on `workflowHasEffort()`. The node control goes through the shared `configSelect` seam with options filtered by `modelEffortLevels`, so the picker cannot offer a level the step's model would drop. `resolvedEffortFor` resolves DOWN only, never up, and every difference is surfaced in prose or a generated comment. No step carrying effort means byte-identical output to before the feature existed
...
A removable block (`LIVE RUN MONITOR` in index.html) that watches up to four directory handles (File System Access API, handles persisted in IndexedDB via the shared `awdIdb` kv, key `monitorDirs`) and renders live run telemetry. Poll-based (`MONITOR_POLL_MS`, paused on `document.hidden`) because browsers ship no file-watch events: each cycle scans every watched tree (bounded by `MONITOR_MAX_DEPTH`/`MONITOR_MAX_FILES`, paths prefixed by watch name), diffs `lastModified`+size snapshots (`monitorDiff`, pure), and re-reads only changed text files under `MONITOR_MAX_BYTES`. The scan honors each directory's own `.gitignore` (`monitorParseIgnore`/`monitorIgnored`, last-match-wins with negations) with `MONITOR_SKIP_DIRS`/`MONITOR_SKIP_FILE` as the fallback for trees without one. A tree that still overflows the file budget is flagged capped and sees a shifting partial sample, so it feeds only co-observed modifications: phantom removals are guarded by `scannedPrefixes` in `monitorDiff` (the snapshot union-grows instead), and phantom new files are absorbed by the baseline/capped filter in `monitorPoll` (snapshot and texts still grow silently). The parsing core is pure functions over strings - `parseToonEntries` (both `## @slug | ISO | status` and `## Step N: Label | ...` header forms), `parseTaskChecklist` (checkbox lines with optional `(@slug)` owner tags), `parseProvisionalTicks` (`t: {task} done` lines), `monitorMergeTasks` (three tiers: confirmed from `tasks.md` ticks, provisional from normalized-substring-matched agent ticks, open), `monitorAgentStatuses` (own file authoritative, a done handoff in `shared.md` wins, orchestrator log lines excluded) - and `monitorScanHandle` accepts any object speaking the handle iterator protocol (fake-handle testable, the `scanRunReportsFromHandle` precedent). `monitorRecompute` locates `tasks.md` at any depth and scopes the run surface to its directory, so one watch serves a multi-repo run (the memory folder is shared) while additional `.workflow` watches add durable-record browsing: the panel viewer renders `.md` through `renderMarkdown` with an `awd:record` header chip. Canvas nodes get live status dots through the guarded `monitorNodeStatus` hook (label-slug matching, `MONITOR_DOT` palette). The emission side is the two-tier progress convention in the memory protocol variants: the orchestrator seeds and individually ticks `tasks.md` (its checklist copied from the durable record when one is kept, else derived from the plan - conditional at generation time), and each sub-agent appends provisional `t:` lines to its own file, preserving the one-writer-per-file invariant.
...
- **Input popup**: Prompts with input config show a modal collecting user context before copying. Fields render in declared order and come in three kinds: `text` (textarea; supports `optional: true` with `fallback` text), `select` (a real dropdown, skinned by the unified `enhanceSelect` enhancer, with a `default` option preselected), and `checkbox` (an inline-labeled toggle that substitutes its `on`/`off` text and never blocks the copy - the shared save-as-markdown-file pattern used by every Documentation prompt and the upgrade prompt's report destination). Each field's `find` token is replaced everywhere it appears in the prompt body. A single-text prompt renders exactly as it always has; the Software & Framework Upgrade prompt is the first to use the multi-field form (a target, four dials, and optional constraints)
