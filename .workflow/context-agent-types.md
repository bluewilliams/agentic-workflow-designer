...
- 2026-08-07: "App Source" renamed "App Under Test" everywhere user-facing - the director caught the Explain row reading like an unconfigured requirement for ordinary workflows ("oh no, I didn't configure the app source"). Emitted headings (all 4 prompt formats), the Explain row (whose OFF reason now says outright: the app-to-explore for test automation, NOT your project source, needed only when App Explorer steps mine selectors), and both advisor rules now share the one name; the SDK's app_source_path identifier stays (code, not prose). Same pass fixed every Explain OFF-reason still pointing at pre-merge section names (UI Context, Repositories incl. only-needed-for-multi-repo framing, rules/docs/verification path rows -> Multi-Repo & Context Paths / App & UI Context) (by direct session)
