...
Follow-on 2 (reroute back-edge target): a review loop's failure branch can now be re-pointed to any work node, not just the immediate reviewed node (e.g. route a Verifier's "Not verified" back to the Planner). Control: a configSelect on the review-loop DECISION node ("On {noLabel}, route back to"), candidates = agent+task nodes excluding review nodes, default/current = the back-edge's live `to`. Implementation is edge-only: setReviewLoopBackTarget(decisionId, targetId) re-points the back-edge (the decision's connection whose label===noLabel) with pushUndo+render; the data-key 'reviewLoopBackTo' is intercepted in the custom-select handler (early return before the normal config write) so it never writes to config. The edge is the single source of truth - generators render the rerouted target by reading the edge, detach is unaffected (restores via the yes branch), and serialization is free. Canvas redraws the back-edge live to the chosen node (Blue's ask, browser-verified: failure curve moves to the Planner; dropdown shows label "On \"Not verified\", route back to" with current=Planner and only work nodes as candidates). Tests 690->697 (+7). No changes to attach/detach/generators/serialization. Independent review: PASS, no Critical/High (confirmed the data-key interception is isolated, the edge re-point is undoable + read consistently everywhere, no duplicate-edge bug). Known pre-existing caveat (not introduced here, noted in backlog): hand-editing a review-loop decision's No Label after attach desyncs the back-edge label - the new code shares the generators' existing label-match assumption.
