...
- intent: per-node Defect Posture selector for the found-defect protocol - a node-config select on carrying roles (coder/backend/frontend/general/debugger/tester/verifier) choosing Balanced (default, byte-identical to the shipped protocol), Record-only (surgical diff: report everything, fix only true blockers), or Aggressive (opportunistically heal the working area, every fix tested and declared); nodeDefectPosture accessor (absent = balanced), variants inside foundDefectNote so all six emission sites follow; reviewer judges declared fixes against the producing step's declared posture; Explain shows the active posture and names the lever.
