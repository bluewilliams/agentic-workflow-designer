...
- 2026-08-06: flagship "Software & Framework Upgrade" prompt added (director-requested, expected high-traffic) as the first entry of "Data & Migrations", renamed "Migrations & Upgrades" to fit both residents (no-compat license; the rename orphans any saved favorite keyed on the old category name - favorites silently unstar, nothing breaks). The prompt is a 4-phase gated upgrade contract: baseline-before-touching (all validation signals recorded as the honesty anchor), research-before-code (official migration guide + breaking changes for EVERY major boundary crossed, live-source verification over stale training data, evidence-grep of actually-used APIs), staged execution (official tooling first, revert-to-last-green on stall, never weaken tests), and verification (baseline matrix + runtime proof + advisory delta + a verdict the matrix must support). Two in-prompt dials with conservative defaults (DEPENDENCY SCOPE minimal|important|everything; API ADAPTATION necessary-only|modernize) + free-text CONSTRAINTS - dials chosen over popup dropdowns so each level carries its definition and the choice stays visible in the copied text. Library now 76 entries. +2 contract tests, 1565 -> 1567 (by direct session)
- 2026-08-07: input popup generalized to REAL multi-field forms (director-requested) - entries may now carry an `inputs` array (text and select kinds, declared order) alongside the legacy single `input` object (normalized via plibInputList; a lone text input renders byte-identical to the classic layout, first textarea keeps the plibInputText id so Cmd+E expand holds). Selects are native elements skinned by the unified enhanceSelect dropdown; each field's find token is replaced at EVERY occurrence (split/join, was first-only). The upgrade prompt became the first consumer: target text + three dropdowns (DEPENDENCY SCOPE minimal/important/everything, API ADAPTATION necessary-only/modernize, NEW REPORT DETAIL full report/summary/verdict only - detail governs the write-up, never what is verified) + optional CONSTRAINTS (fallback "none stated", outrank every dial). Prompt hardened for the whole spectrum (old C++ to MUI): target-existence sanity check (never invent a version; EOL/newer-LTS flagged), build-file version detection across ecosystems (CMake/conan/vcpkg, gradle, go.mod, pubspec), compiler/language-standard prerequisites, rendering signal mandated for UI-library upgrades (styling regressions escape unit tests), legacy-repo safety net built FIRST when no signals exist, published-library leak check (re-exports/peers/ABI can force a major bump of the host), build scripts/CI/native files in scope. Dash guard extended over inputs arrays (labels, placeholders, finds, option values). +4 tests net, 1567 -> 1571 (by direct session)
- 2026-08-07: upgrade prompt examples neutralized at the director's request - Angular replaces the mobile-framework watermark across placeholder/preamble/tooling/doctor examples (ng update, ng version, rails app:update joins the tooling list), and the constraints placeholder's do-not-touch example is now "the legacy reporting module". Same example shapes, no behavior change (by direct session)
- 2026-08-07: report delivery made the user's call - a fourth dropdown REPORT DESTINATION (chat default / markdown file: docs/upgrades/upgrade-<target>-<date>.md on the upgrade branch so the record travels with the change; detail none writes nothing rather than an empty file), and REPORT DETAIL gains a true opt-out level none (no write-up; the one-line verdict still lands, caveats named in it - an upgrade may end quietly, never dishonestly). Popup is now 6 fields (target, 4 dropdowns, optional constraints) (by direct session)
- 2026-08-07: pre-commit review fleet caught two checkbox-feature defects, both fixed - Deep Documentation Generator and Codebase Onboarding Guide still carried their baked write-to-file lines, contradicting the unchecked checkbox's deliver-in-reply text; both now follow the API-generator pattern (default CHECKED, historical target preserved as the on text, baked line removed - the checkbox owns the decision). And the popup's single-input heuristic treated a lone checkbox as classic single-text layout, rendering its label twice (as desc AND inline); a lone checkbox now gets the generic desc. The generic save-to-file literal hoisted to PLIB_DOC_SAVE_CHECKBOX (3 consumers; the 3 historical file-writers carry custom variants). Contract tests migrated to the checked-set model (by direct session)
- 2026-08-07: third field kind `checkbox` (inline label, substitutes its on/off text at the find token, never blocks the copy) - the shared save-as-markdown-file pattern: all 6 Documentation prompts gained it feeding a new [output destination] body token (off = deliver in reply; API Documentation Generator defaults CHECKED with its historical `.claude/docs/api.md` target preserved as the on text; the 4 legacy single-input docs entries migrated to inputs arrays), and the upgrade prompt's REPORT DESTINATION dropdown became the same checkbox (on 'markdown file' / off 'chat' feed the unchanged body token - pure input-kind swap, zero prompt-body change). Dash guard covers on/off surfaces. 1571 -> 1574 (by direct session)
