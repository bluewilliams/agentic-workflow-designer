...
Repo Context Paths has a third sticky group, "Verification instructions (verifier / tester owned)": an add-chip list of filenames or paths (bare names searched recursively per repo in scope) plus an "Unmet required verifications fail the run" checkbox (default ON; armed only while mandates exist - `syncVerifyFailRunUI()` disables it with an explanatory hint when `hasVerifyMandates()` is false, state kept sticky so the preference re-arms with the first mandate), persisted like rulesPaths and surviving New Workflow. When the list is non-empty, `verifyInstructionsHint()` emits an orchestrator beat into all five export formats (workflow, sub-agents, teams, claude.ai, SDK as a comment block) carrying discovery, root-vs-nested blast-area scoping, MUST/SHOULD severity interpretation with the checkbox posture as default, and posture-correct fail-the-run vs report-only language - and routes the repo's verification mandates to exactly ONE step. `resolveVerifyOwnerId()` picks the workflow's OWNING verification step (the last verifier-type node in execution order, else the last tester); `verifyInstructionsStepHint(node)` emits the per-step contract (executed-with-result / not-applicable-with-reason / blocked-with-reason, surfaced into the gate verdict, record Verify section and Verification ledger, and the awd:run fence's `blockedVerifications` array (an optional field documented beside `grounded`, emitted only when mandates exist), never silently passing; explicitly IN ADDITION to the step's own verification method, never a substitute - these are REPO-mandated checks) ONLY to that owner - reviewers audit reported outcomes and never receive the execute contract, and a tester upstream of a verifier receives nothing (a step that never hears of an expensive mandate cannot duplicate it); `openSpecContextBlock` forwards the paths compactly. With the list empty, every surface emits byte-identically to before - silence is structural. The files mandate what must be proven, never which tools may be used.
...
- 2026-08-07: ownerless-workflow gap closed (director probed whether verifiers will honor the mandates; audit found the beat routed files to a Verifier-else-Tester that a workflow may not have) - the beat now carries an explicit fallback (the orchestrator executes the applicable checks itself at close-out under the same posture; configured verification never goes unexecuted for lack of an owner), advisor rule (j) surfaces the ownerless configuration, and the SDK comment's routing drift fixed (it sent mandates to tester/verifier/REVIEWER briefs - reviewers audit outcomes, never execute; it now names the owning-step routing). +3 tests, 1588 -> 1591 (by direct session)
- 2026-08-07: inline verification notes (director-requested) - a free-text box beside the file chips (state.verifyNotes; sticky prefs like the paths; Cmd+E expandable) feeds the SAME machinery end to end: one hasVerifyMandates() predicate gates every consumer (beat, owner step hint, SDK comment, OpenSpec context, Explain, attention pulse, advisor rule j); the beat renders notes with the same MUST/SHOULD classification and posture while the file-only lines stay gated on paths; notes ride the handoff envelope with adopt-only-if-empty semantics. Placeholder teaches severity by example (an E2E MUST + a Lighthouse SHOULD; mutation testing stays the help modal's exotic exemplar). Verifier nodes (only - director-scoped) gained a config-panel mandate status line: unconfigured points at the section (deep link), the owner names its files/notes and posture, a non-owner names who owns and why. Repair note: the 2026-08-07 ownerless History line was originally spliced into every code fence of this record by a buggy append (anchored on a bare fence line) and committed that way; fences restored and the line relocated here (by direct session)
- 2026-08-22: run-evidenced arming + structured telemetry (a real two-repo epic run reported Pass with the checkbox checked and NO mandates configured: the posture is a modifier on mandates, every emission site gates on hasVerifyMandates(), so the checked box was inert while reading as an active guarantee) - syncVerifyFailRunUI() now disables the checkbox with an explanatory hint while mandates are absent (state sticky; synced via renderVerifyRows, setVerifyNotes, and startup), and the blocked-check surface moved from free-text fence notes to a structured awd:run blockedVerifications array (documented beside grounded, gated on mandates) so the telemetry loop can count blocked required checks instead of parsing prose. Review pass closed the loop end to end: ingestRunReport stores the array (newest-last, capped 5, like recentNotes), genTuningPrompt surfaces it as its own section, and the OpenSpec apply instruction documents the field under the same gate - without the read side, moving evidence out of notes would have been a telemetry regression. The advisor's suggested validateWorkflow warning was deliberately NOT taken: verifyFailRun defaults ON, so the warning would permanently nag every fresh install; the disabled control states the same truth without noise. Test hygiene: resetState never reset verifyNotes (a mid-test failure could leak armed mandates into every later suite) - reset at the root now, plus the DOM describe's afterEach. 1614 -> 1618 (by direct session)
- 2026-08-07: the Verifier status line refreshes IN PLACE (director caught the staleness: adding a file + notes only showed after reselecting the node) - verifyStatusHtml extracted as the single builder, refreshVerifyStatusLine hooks the three sidebar seams that change what it reports (path mutators via renderVerifyRows, setVerifyNotes, setVerifyFailRun since the line names the posture); no panel rebuild, one element updated. +1 test, 1601 -> 1602 (by direct session)
