...
- 2026-08-07: the health badge gained an ATTENTION tier (director-requested): a narrow advisorAttention() roster - deliberately only configuration contradictions that change what a run reports; sole member today is verify-mandates-with-no-owner (shared verifyOwnerlessMisconfig predicate with advisor rule j) - pulses the badge gently (CSS keyframes, silenced under prefers-reduced-motion) with a title cue, acknowledged-on-view (opening the review stops the pulse until the attention SET changes; session-scoped, never nags). Issues-only badge semantics preserved: the checkmark stays green - attention is not an issue. Each future roster entry is an individual design decision, never a pattern to extend by drift. +3 tests, 1591 -> 1594 (by direct session)
- 2026-08-07: pulse staleness fixed same-day (director caught it live: clearing the verify file left the badge pulsing) - updateValidation only ran on canvas renders, and path edits do not render the canvas; renderVerifyRows (the single seam all three path mutators funnel through) now refreshes the badge, so the pulse tracks config edits live in both directions. +1 test, 1594 -> 1595 (by direct session)
- 2026-08-07: the suite now lints the durable records themselves (born from a real committed incident: a scripted History append anchored on a bare fence line spliced prose into every fence of verification-instructions.md) - an async test fetches every record listed in _index.md over the test server and fails CI on glued-prose fence lines, unbalanced fence counts, missing listed records, or an unparseable awd:record fence; file:// contexts skip gracefully (CI is the enforcer), and the guard is canary-proven (a deliberately corrupted record failed the suite by name before restore). All 99 records swept clean first. +1 test, 1600 -> 1601 (by direct session)
- 2026-08-22: the memory protocol's orchestrator surfaces gained the authoritative-report-from-disk contract and the @orchestrator decisions log - amended by epic-run-feedback (run-evidenced; see that record)
- 2026-08-22: the direction-(d) ruling (our prompt runs deliberately do NOT read OpenSpec docs; converge-on-one-index instead) is PARTIALLY SUPERSEDED by director decision - grounding now also scans openspec/ documents directly, because OpenSpec-native tooling never writes our breadcrumbs, so convergence could not reach repos run by it. The one-index principle stands for OUR records; the direct scan is bounded exactly like the index scan (names as the index, grep to narrow, few files opened, zero-match costs two listings). Implemented in groundingLookupSteps as the openSpecDocs step (by direct session)
