...
- The private hygiene grep (CLAUDE.local.md, gitignored - the pattern lives only there) -> exit 1 on all changed files. No em dashes in any addition. (Sanitized 2026-07-03: this record originally quoted the pattern verbatim - the hygiene rule leaked through its own documentation; tracked files now reference the check, never quote it.)
...
- 2026-07-03: found-defect protocol gains the per-node Defect Posture selector (balanced byte-identical default, record-only, aggressive) - Current behavior amended in place, status stays current (by defect-posture)
