...
- 2026-08-25: review fixes on the richness pass - evidence blocks now match the emitting format's heading level (subagent/teams/claude emit these blocks at ##, prompt/SDK at the ### defaults; _exH threads it into repoBlock/appSourceBlock/deliveryBlock at both workflow and output-node level), the dead winnerHeading const dropped (its only consumer became deliveryBlock), the three path-list reasons filter blank entries (hand-edited prefs blobs), and exInlineMd unified onto the shared inlineFmt with code-spans-first ordering so backticked glob literals never bold. 1637 -> 1640 (by direct session). Second review pass: the heading-level ternary extracted into one shared explainHeadingLevel(format) (was encoded twice), and the Model row's redundant probe ternary dropped (exRow blanks probe/full itself when not emitted).
