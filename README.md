...
Prompts that need context (like "what file to analyze") show an input popup before copying so the prompt is ready to paste with no editing. Bigger prompts get real controls: the Software & Framework Upgrade prompt asks for the target and version, then hands you four dropdowns (how many dependencies may move, whether deprecated APIs get modernized, how much report you want to read afterwards - down to none - and whether that report lives in chat or lands as a markdown file in the repo) - every default is the cautious one, so leaving them alone is a decision, not an accident. Star your favorites and they float to the top.
