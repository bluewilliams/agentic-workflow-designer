...
- **Model** - Fable 5, Opus 5, Opus 4.8, Sonnet 5, Sonnet 4.6, and Haiku 4.5, plus 1M context variants for every family except Haiku. The default is Opus 5; set a different default in the sidebar or override per-node as needed. Max plan users get 1M context by default. API and Pro users can select 1M variants for research-heavy or long-running agents where the extra context window makes a difference. The newest model in each family emits the short alias (`opus`, `sonnet`, `fable`, `haiku`); older ones emit their full API id, so a step pinned to an earlier generation stays on it
